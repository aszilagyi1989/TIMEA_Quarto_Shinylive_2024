...
telepules_MAP <- read.csv("https://raw.githubusercontent.com/aszilagyi1989/TIMEA_Quarto_Shinylive_2024/refs/heads/main/JSon/megye_pbi.csv", sep = ";", row.names = NULL)
...
telepules_MAP$VALUE <- as.numeric(telepules_MAP$VALUE)
  ...
  output$mymap <- renderPlotly({
    ...
    # tryCatch({
      ...
      map <- ggplot(data = subsetted()) + geom_sf(alpha = (input$alpha / 100), aes(text = Megyenév, fill = VALUE))
      ggplotly(map) %>%
